...
import { LogOut, ChevronDown, ShieldCheck, Eye, Store } from 'lucide-react';
...
export const AdminHeader: React.FC = () => {
  ...
  return (
    <header className="h-16 bg-white/90 backdrop-blur-md border-b border-slate-100 px-6 flex items-center justify-between sticky top-0 z-40 select-none font-sans">
      {/* Left: Greeting */}
      ...
      <div className="flex items-center gap-3">
        ...
        {isSuperAdminUser && (
          <div className="flex items-center gap-1.5 bg-amber-50/80 hover:bg-amber-100/80 text-amber-900 border border-amber-200/80 text-xs px-2.5 py-1.5 rounded-xl transition shadow-2xs">
            ...
            <select
              ...
              className="bg-transparent font-bold text-amber-950 text-xs border-none outline-none cursor-pointer focus:ring-0"
        ...
        <Link
          to="/"
          className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 hover:bg-amber-500 hover:text-white text-slate-800 rounded-xl text-xs font-bold transition shadow-2xs border border-slate-200/80"
        ...
        <div className="relative">
          <button
            ...
            className="flex items-center gap-2.5 p-1.5 hover:bg-slate-50 rounded-2xl transition border border-transparent hover:border-slate-200"
          >
