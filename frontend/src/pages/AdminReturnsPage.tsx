...
export const AdminReturnsPage: React.FC = () => {
  ...
  const handleSeedReturnsData = async () => {
    setSeedLoading(true);
    try {
      await apiClient('/api/reports/seed', { method: 'POST' });
      await fetchReturns();
    } catch (err: any) {
      console.error('Error seeding return mock data:', err);
      alert(err.message || 'Không thể tạo dữ liệu đổi trả mẫu.');
    } finally {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              onClick={handleSeedReturnsData}
              disabled={seedLoading}
              className="px-3.5 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold text-xs rounded-xl shadow-2xs transition flex items-center gap-1.5 border border-indigo-200/80 disabled:opacity-50"
              title="Khởi tạo danh sách các đơn đổi trả mẫu"
            >
              {seedLoading ? <Loader2 className="w-3.5 h-3.5 animate-spin text-indigo-600" /> : <Sparkles className="w-3.5 h-3.5 text-indigo-600" />}
              Thêm đơn đổi trả mẫu
            </button>
