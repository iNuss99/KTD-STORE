...
export const AdminOrdersPage: React.FC = () => {
  ...
  const handleCreateSingleOrder = async () => {
    ...
    try {
      await apiClient('/api/reports/seed-single', { method: 'POST' });
      showSuccess('Tự động tạo đơn hàng thành công!', 'Đã thêm 1 đơn hàng mới vào danh sách.');
      await fetchOrders();
    } catch (err: any) {
      showError('Không thể tạo đơn hàng', err.message || 'Có lỗi xảy ra khi tạo đơn hàng mới.');
    } finally {
    ...
    <div className="flex flex-col font-sans">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* Search bar & Seed Single Order button */}
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <button
              onClick={handleCreateSingleOrder}
              disabled={singleOrderLoading}
              className="px-3.5 py-2 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-bold text-xs rounded-xl shadow-sm hover:shadow-md transition flex items-center gap-1.5 border border-amber-400/20 disabled:opacity-50 shrink-0"
              title="Tự động khởi tạo 1 đơn hàng mẫu mới vào danh sách"
            >
              {singleOrderLoading ? (
