      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 border-l-4 border-l-emerald-500 hover:shadow-md hover:-translate-y-0.5 transition duration-200 flex flex-col justify-between group">
            <div className="flex items-start justify-between">
              ...
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-400 to-emerald-600 text-white flex items-center justify-center shrink-0 shadow-sm shadow-emerald-200 group-hover:scale-105 transition-transform">
          ...
          {/* KPI 2: Đơn thành công */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 border-l-4 border-l-blue-500 hover:shadow-md hover:-translate-y-0.5 transition duration-200 flex flex-col justify-between group">
            <div className="flex items-start justify-between">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Đơn thành công</p>
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-blue-400 to-blue-600 text-white flex items-center justify-center shrink-0 shadow-sm shadow-blue-200 group-hover:scale-105 transition-transform">
          ...
          {/* KPI 3: Đơn chờ xử lý */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 border-l-4 border-l-amber-500 hover:shadow-md hover:-translate-y-0.5 transition duration-200 flex flex-col justify-between group">
            <div className="flex items-start justify-between">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Đơn chờ xử lý</p>
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-amber-600 text-white flex items-center justify-center shrink-0 shadow-sm shadow-amber-200 group-hover:scale-105 transition-transform">
          ...
          {/* KPI 4: Tồn kho thấp */}
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 border-l-4 border-l-rose-500 hover:shadow-md hover:-translate-y-0.5 transition duration-200 flex flex-col justify-between group">
            <div className="flex items-start justify-between">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Tồn kho thấp (≤ 5)</p>
              <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-rose-400 to-rose-600 text-white flex items-center justify-center shrink-0 shadow-sm shadow-rose-200 group-hover:scale-105 transition-transform">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-7 hover:shadow-md transition duration-200 select-none">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
              <div className="bg-slate-50 border border-slate-100 px-3.5 py-2 rounded-xl flex items-center gap-2.5">
              ...
              <div className="bg-slate-50 border border-slate-100 px-3.5 py-2 rounded-xl flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <div className="text-xs sm:text-sm font-extrabold text-slate-900">{totalPeriodOrders} đơn</div>
                </div>
              </div>
              <div className="bg-slate-50 border border-slate-100 px-3.5 py-2 rounded-xl flex items-center gap-2.5">
          ...
          {revenueData.length === 0 ? (
            ...
            <div className="w-full overflow-x-auto pb-2 pt-1">
              <div
                ...
                style={{ minWidth: revenueData.length > 10 ? `${Math.max(revenueData.length * 28, 650)}px` : '100%' }}
              >
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={revenueData} margin={{ top: 20, right: 15, left: 0, bottom: 0 }}>
                    <defs>
                    ...
                    <XAxis dataKey="period" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} dy={12} />
                    <YAxis yAxisId="left" axisLine={false} tickLine={false} tick={{ fill: '#64748b', fontSize: 11, fontWeight: 600 }} tickFormatter={(value) => `${(value / 1000000).toFixed(1)}M`} />
                    <YAxis yAxisId="right" orientation="right" axisLine={false} tickLine={false} tick={{ fill: '#94a3b8', fontSize: 11, fontWeight: 600 }} />
                    <Tooltip content={<CustomLuxuryTooltip />} />
                    <Area yAxisId="right" type="monotone" dataKey="orderCount" fill="url(#colorOrderArea)" stroke="none" />
                    <Bar yAxisId="left" dataKey="revenue" name="Doanh thu" fill="url(#colorRevenueGradient)" radius={[8, 8, 0, 0]} maxBarSize={36} />
                    <Line yAxisId="right" type="monotone" dataKey="orderCount" name="Số lượng đơn" stroke="#f59e0b" strokeWidth={3} dot={{ r: 4, fill: '#f59e0b', strokeWidth: 2, stroke: '#ffffff' }} activeDot={{ r: 6, fill: '#d97706', stroke: '#ffffff', strokeWidth: 2 }} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </div>
                        ...
                        <div className="flex items-center gap-3 flex-1 min-w-0">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                              ...
                              <div className="h-1.5 w-full max-w-[80px] sm:max-w-[120px] bg-slate-100 rounded-full overflow-hidden">
              ...
              <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-slate-50">
                  {lowStockVariants.length === 0 ? (
                    <tr>
                      <td colSpan={4} className="text-center py-10 text-emerald-600 font-medium">
                    ...
                    lowStockVariants.map((variant) => (
                      <tr key={variant.id} className="hover:bg-slate-50 transition group">
                        ...
                        <td className="py-2.5 px-3 text-center">
                          <button className="px-2 py-1 text-[10px] font-bold bg-white border border-slate-200 text-slate-600 rounded shadow-sm hover:bg-slate-50 hover:text-indigo-600 transition inline-flex items-center gap-1">
        ...
        {/* ── Hàng 4: Hiệu suất Nhân viên (full width) ── */}
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6 hover:shadow-md transition duration-200">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="text-base font-bold text-slate-900 flex items-center gap-2">
