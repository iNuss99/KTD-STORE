...
export const ToastProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <ToastContext.Provider value={{ showToast, showSuccess, showError, showInfo, showWarning }}>
      ...
      {activeModal && (() => {
        // Gọi getTheme 1 lần duy nhất — tránh 3× switch-case mỗi render
        const theme = getTheme(activeModal.type);
        return (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 font-sans select-none animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-6 text-center border border-slate-100 relative animate-in zoom-in-95 duration-200 space-y-4">
              ...
              {/* Icon Circle */}
              <div className={`mx-auto w-16 h-16 rounded-2xl flex items-center justify-center shadow-inner ${theme.bgIcon}`}>
                {theme.icon}
              </div>
              ...
              <div className="space-y-1.5">
                <h3 className="text-lg font-extrabold text-slate-900 leading-tight">
                  {activeModal.title || theme.defaultTitle}
                </h3>
                {activeModal.message && (
              ...
              <div className="pt-2">
                <button
                  onClick={handleClose}
                  className={`w-full py-3 px-5 rounded-2xl font-bold text-xs shadow-md transition ${theme.btnBg}`}
                >
                  Đồng ý / Đã hiểu
                </button>
              </div>
